		...
		const float *restrict const fd_coeff, const int step)
{
	...
	int sx;
	int sy;
	const int x2pad = nx - nxi - 8;

	for (i = 8 * nx + 8; i < (ny - 8) * nx - x2pad; i++) {
		f_xx[i] = 2 * fd_coeff[0] * f[i];
	}

	for (k = 1; k < 9; k++) {
		cblas_saxpy(nx * (ny - 16) - 8 - x2pad, fd_coeff[k], f + 8 * nx + 8 + k, 1, f_xx + 8 * nx + 8, 1);
		cblas_saxpy(nx * (ny - 16) - 8 - x2pad, fd_coeff[k], f + 8 * nx + 8 - k, 1, f_xx + 8 * nx + 8, 1);
		cblas_saxpy(nx * (ny - 16) - 8 - x2pad, fd_coeff[k], f + 8 * nx + 8 + k * nx, 1, f_xx + 8 * nx + 8, 1);
		cblas_saxpy(nx * (ny - 16) - 8 - x2pad, fd_coeff[k], f + 8 * nx + 8 - k * nx, 1, f_xx + 8 * nx + 8, 1);
	}
