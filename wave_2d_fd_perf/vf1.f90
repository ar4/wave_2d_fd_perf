...
contains
  ...
  subroutine step(f1, f2, model_padded2_dt2, dx, sources,              &
    ...
    if (even) then
      call step_inner(f2, f1, model_padded2_dt2, sources,              &
              sources_x, sources_y, step_idx, fd_coeff)
    else
      call step_inner(f1, f2, model_padded2_dt2, sources,              &
              sources_x, sources_y, step_idx, fd_coeff)
    end if
    end do
  ...
  subroutine step_inner(f, fp, model_padded2_dt2, sources,             &
    ...
    do k = 1, 8
    f_xx = f_xx + fd_coeff(k + 1) * (f(j, i + k) + f(j, i - k) +       &
            f(j + k, i) + f(j - k, i))
    end do
    fp(j, i) = (model_padded2_dt2(j, i) * f_xx + 2 * f(j, i) - fp(j, i))
    end do
    end do
